...
`06faa1c2a74599656dffcf0b67102f532ba951a3`；当前代码仍以该提交为 HEAD，但包含本 change 的
工作树修改，因此结果同时记录三个核心源文件的 SHA-256 指纹，避免把相同 HEAD 误认为相同代码。
...
| current | `06faa1c2a74599656dffcf0b67102f532ba951a3` + worktree diff | `551e3900…1c01eb74` |
...
| 完整步骤组 | 0/8 | 6/8 |
| 完整管线 P50 | 0.384 ms | 0.964 ms |
| 完整管线 P95 | 0.515 ms | 1.431 ms |
...
变为 3/3。当前管线增加 auto-merge 与 step-chain 后，本冻结集 P50/P95 分别增加 0.580 ms
和 0.916 ms；这是模型无关、本机、80 次完整后处理调用的相对开销。
...
| ---: | ---: | ---: | ---: | ---: |
| 10 | 0.2917 | 2/8 | 0.321 ms | 0.631 ms |
| 15 | 0.5417 | 4/8 | 0.769 ms | 1.221 ms |
| 20 | 0.9167 | 6/8 | 0.964 ms | 1.431 ms |
...
完整步骤组增加 2 个，P95 增加 0.210 ms；相对 10 有 6 胜、0 负、2 平。模型输入成本仍由
