...
```bash
uv run python tests/e2e/deepdoc_parse_metadata/validate_http_parse_metadata.py
```
...
curl -X POST http://localhost:8000/documents/upload \
  -F "file=@tests/fixtures/documents/国电电力.pdf" \
...
```bash
curl -X POST http://localhost:8000/documents/upload \
  -F "file=@tests/fixtures/documents/test_docx.docx" \
...
curl -s -X POST http://localhost:8000/documents/upload \
  -F "file=@tests/fixtures/documents/国电电力.pdf" > /dev/null
sleep 2
...
echo "上传 DOCX..."
curl -s -X POST http://localhost:8000/documents/upload \
  -F "file=@tests/fixtures/documents/test_docx.docx" > /dev/null
sleep 2
...
保存为 `tests/e2e/deepdoc_parse_metadata/quick_http_parse_metadata.sh`，执行：
```bash
