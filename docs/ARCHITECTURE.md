...
| Multilevel fallback | Implemented, default disabled; real validation pending | Agent-completable M0–M9 and M10 unit regressions exist; `RAG_FALLBACK_ENABLED=false`; real UX/evaluation/tuning remain in `openspec/changes/rag-multilevel-fallback/` |
...
- Anchor routing lacks an atomic capability configuration and shared extraction/normalization contract; multilevel fallback real UX, evaluation, tuning, and rollout evidence remain incomplete, and the grouped env example is validation-only.
