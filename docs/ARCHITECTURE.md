...
status: current
verified_commit: ba5947c4ed8c4e38f31ffa6834e70e561f1f0a41
...
`backend/rag/comprehensive_postprocess.py` resolves a frozen, versioned strategy composition. `quality_first_v1` is the production profile and `eval_no_crossencoder_v1` is evaluation-only. Unknown profiles atomically fall back to `quality_first_v1`. Output-candidate and CrossEncoder-pair budgets are allocated independently across all branches; when a branch pair quota is smaller, reranked pairs lead and the unpaired Milvus-ranked tail fills the remaining output quota (`RAG-INTENT-F021`). CrossEncoder failures returned through rerank metadata are branch degradations just like raised failures: usable Milvus candidates remain, while branch diagnostics record the error (`RAG-INTENT-F024`). The Chat Agent still invokes `search_knowledge_base(query)` once; it never receives or iterates sub-queries, and this capability has no multi-turn mode. Sub-query rewrite remains outside this change as fallback Level 1 work.
