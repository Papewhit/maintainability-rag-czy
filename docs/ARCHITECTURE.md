...
| Multilevel fallback | Implemented, default disabled; manual M8 UX validated, real evaluation pending | Agent-completable M0–M9 and M10 unit regressions exist; `RAG_FALLBACK_ENABLED=false`; `VAL-RAG-FALLBACK-001` closes M8.5, while real query-set evaluation and tuning remain in `openspec/changes/rag-multilevel-fallback/` |
...
- Anchor routing lacks an atomic capability configuration and shared extraction/normalization contract; multilevel fallback real evaluation, tuning, and rollout evidence remain incomplete, while its narrow manual M8.5 UX gate passed under `VAL-RAG-FALLBACK-001`; the grouped env examples remain validation-only.
