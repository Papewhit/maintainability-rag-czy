...
status: current
verified_commit: 961eb0f29677a23d2a0bcf5ff4da720ab701fa79
...
This is not yet a unified anchor/fallback contract. Query preparation, confidence, and chunk normalization use different anchor grammars and surface normalization, and precise confidence re-extracts from raw query. The multilevel graph preserves typed scope and Level 0 semantic retrieval input across Level 1/2 rounds. Level 3 builds a typed delivery contract without retrieval or a template-generation LLM; its deterministic renderer is the sole producer of the legacy `level3_answer` projection. The contract distinguishes partial synthesis, full-coverage low confidence, baseline-only, no-evidence, and precise-insufficient modes and binds per-dimension evidence refs to the final top-k. Only partial final-top-k coverage (`0 < X < Y`) authorizes the existing answer model to generate source-bound, per-dimension partial answers, with uncovered and cross-dimension conclusions prohibited; full-coverage/low-confidence, baseline-only, no-evidence, and precise-insufficient modes remain evidence-only. forced-preload and optional-tool delivery consume the same typed contract and renderer. After intent parsing freezes the typed plan, one explicit route event identifies the effective precise or comprehensive path; safe degradation reports the actual precise path without exposing provider errors. Structured fallback events and ordered aggregate comprehensive events (fanout, branch processing, merge, and shared postprocess) then feed the default-collapsed frontend path display; parallel workers do not emit user events. The compatibility switches, legacy timeout mapping, deprecation warning, and migration guide remain implemented. The narrow manual M8.5 UX gate passed under `VAL-RAG-FALLBACK-001`; mixed synthetic/authorized-real evaluation, project-level thresholds, reference budget tuning, and local rehearsal are owned by the planned [fallback activation change](../openspec/changes/rag-multilevel-fallback-activation/). The broader configuration and extraction gaps remain governed by [KI-RAG-0006](known-issues/anchor-capability-configuration.md) (`RAG-INTENT-F034`, `RAG-INTENT-F035`).
