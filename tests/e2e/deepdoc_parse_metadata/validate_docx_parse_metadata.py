#!/usr/bin/env python3
"""Minimal DeepDoc parse metadata validation for DOCX."""
...
print("=" * 60)
print("DeepDoc Parse Metadata Minimal Validation (DOCX only)")
print("=" * 60)
...
try:
    result = service.upload_document("test_deepdoc_parse_metadata_docx.docx", content)
...
try:
    meta = db.query(DocumentParseMeta).filter(
        DocumentParseMeta.document_id == "test_deepdoc_parse_metadata_docx.docx"
    ).first()
    ...
    print("\n" + "=" * 60)
    print("[PASS] DeepDoc parse metadata validation complete!")
    print("=" * 60)
    print("\nValidation results:")
