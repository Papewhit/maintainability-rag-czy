#!/bin/bash
# DeepDoc parse metadata quick HTTP validation script
# 要求：服务已在 localhost:8000 启动

set -e

echo "============================================================"
echo "DeepDoc parse metadata 快速验证"
echo "============================================================"
...
curl -s -X POST http://localhost:8000/documents/upload \
  -F "file=@tests/fixtures/documents/国电电力.pdf" > /tmp/deepdoc_parse_metadata_pdf_upload.json
...
curl -s http://localhost:8000/admin/documents/国电电力.pdf/parse_meta > /tmp/deepdoc_parse_metadata_pdf_meta.json
...
cat /tmp/deepdoc_parse_metadata_pdf_meta.json | python -m json.tool

# 验证 PDF
python -c "
import sys, json
with open('/tmp/deepdoc_parse_metadata_pdf_meta.json') as f:
    data = json.load(f)
...
echo "上传 DOCX..."
curl -s -X POST http://localhost:8000/documents/upload \
  -F "file=@tests/fixtures/documents/test_docx.docx" > /tmp/deepdoc_parse_metadata_docx_upload.json
...
curl -s http://localhost:8000/admin/documents/test_docx.docx/parse_meta > /tmp/deepdoc_parse_metadata_docx_meta.json
...
cat /tmp/deepdoc_parse_metadata_docx_meta.json | python -m json.tool

# 验证 DOCX
python -c "
import sys, json
with open('/tmp/deepdoc_parse_metadata_docx_meta.json') as f:
    data = json.load(f)
...
echo ""
echo "============================================================"
echo "✓ DeepDoc parse metadata 自动化验证完成！"
echo "============================================================"
