...
from ._utils import get_model_dir
import jieba
from ._recognizer import Recognizer


def _is_person_name(word: str) -> bool:
    """Heuristic: likely a Chinese person name (2-3 chars, all CJK)."""
    return 2 <= len(word) <= 3 and all('一' <= c <= '鿿' for c in word)
...
class TableStructureRecognizer(Recognizer):
    ...
    def __init__(self):
        try:
            super().__init__(self.labels, "tsr", get_model_dir())
        except Exception:
    ...
    def blockType(b):
        ...
        for p, n in patt:
            if re.search(p, b["text"].strip()):
                return n
        tks = [t for t in jieba.cut(b["text"]) if len(t) > 1]
        if len(tks) > 3:
            if len(tks) < 12:
                return "Tx"
            else:
        ...
        if len(tks) == 1 and _is_person_name(tks[0]):
            return "Nr"
