...
from ._utils import get_model_dir, is_english as _is_english
from ._ocr import OCR
from ._recognizer import Recognizer
from ._layout_recognizer import LayoutRecognizer4YOLOv10 as LayoutRecognizer
from ._table_structure_recognizer import TableStructureRecognizer
import jieba
...
# ---------------------------------------------------------------------------
# Replacing rag_tokenizer calls (swxy-specific, now using jieba)
# ---------------------------------------------------------------------------


def _is_chinese_char(c: str) -> bool:
    """True if the character is a CJK Unified Ideograph."""
    return len(c) == 1 and "一" <= c <= "鿿"


def _tokenize(text: str) -> list[str]:
    """Tokenize text into words.

    Uses jieba for Chinese text, regex word-split for English.
    Returns a list of non-empty tokens.
    """
    if not text:
        return []
    if any(_is_chinese_char(c) for c in text[:50]):
        return [t for t in jieba.cut(text) if t.strip()]
    # English path: simple word extraction
    return [t for t in __import__("re").findall(r"[a-zA-Z0-9]+", text) if t]


def _tag_noun(word: str) -> bool:
    """Check if *word* is likely a noun using jieba POS tagging.

    Falls back to a simple heuristic if jieba.posseg is unavailable.
    """
    if not word:
        return False
    try:
        ...
        # Heuristic: words that start with uppercase or are all-alpha
        # are more likely to be nouns in technical documents.
        return bool(__import__("re").match(r"[A-Z]", word[0]))
...
class RAGFlowPdfParser:
    ...
    def _updown_concat_features(self, up, down):
        ...
        LEN = 6
        tks_down = _tokenize(down["text"][:LEN])
        tks_up = _tokenize(up["text"][-LEN:])
        tks_all = up["text"][-LEN:].strip() \
                  + (" " if re.match(r"[a-zA-Z0-9]+",
                                     up["text"][-1] + down["text"][0]) else "") \
                  + down["text"][:LEN].strip()
        tks_all = _tokenize(tks_all)
        fea = [
            ...
            len(tks_down) == 1 and _tag_noun(tks_down[0]),
            len(tks_up) == 1 and _tag_noun(tks_up[0])
        ]
        return fea
    ...
    def _table_transformer_job(self, ZM):
        ...
        imgs, pos = [], []
        tbcnt = [0]
        MARGIN = 10 # 裁剪出来的图表四周延伸 10px，以免漏掉边界上的组件
        self.tb_cpns = []
        ...
        for p, tbls in enumerate(self.page_layout):  # for page
            ...
            for tb in tbls:  # for table
                ...
                # 图片是由 72*zoomin 分辨率转换来的，需要乘以 zoomin 确保裁切位置无误
                # 设定 zoomin 使渲染分辨率增加，取得更好的识别效果
                left *= ZM
                top *= ZM
                right *= ZM
                bott *= ZM
    ...
    def _text_merge(self):
        ...
        while i < len(bxs) - 1:
            ...
            if b.get("layoutno", "0") != b_.get("layoutno", "1") or b.get("layout_type", "") in ["table", "figure",
                ...
                i += 1
                continue
            # y方向上相距较近（小于 1/3 字高）的文本块更可能是同一行的，可以合并
    ...
    def _merge_with_same_bullet(self):
        ...
        while i + 1 < len(self.boxes):
            ...
            if b["text"].strip()[0] != b_["text"].strip()[0] \
                    ...
                    or _is_chinese_char(b["text"].strip()[0]) \
                    or b["top"] > b_["bottom"]:
                i += 1
                continue
            b_["text"] = b["text"] + "\n" + b_["text"]
    ...
    def _extract_table_figure(self, need_image, ZM,
        ...
        while i < len(self.boxes):
            ...
            # 如果包含标题或者 caption 等重要信息，不应该被合并到正文
        ...
        while i - 1 >= 0:
            ...
            k, bxs = tbls[i]
            i -= 1
            # 如果前一个块的 layoutno 在 nomerge_lout_no 中，说明它包含标题或者 caption 等重要信息，不应该被合并
            if k0 in nomerge_lout_no:
                continue
            # 如果两个表格在同一个页面上，不需要合并
            if bxs[0]["page_number"] == bxs0[0]["page_number"]:
                continue
            # 如果两个表格不在连续页面上，不应该被合并
            if bxs[0]["page_number"] - bxs0[0]["page_number"] > 1:
                continue
            mh = self.mean_height[bxs[0]["page_number"] - 1]
            # 如果两个表格在 Y 方向上相距较远，说明它们不太可能是同一个表格的不同部分，不应该被合并
            if self._y_dis(bxs0[-1], bxs[0]) > mh * 23: # 23 是经验值
                continue
        ...
        # cropout function to crop figure/table and add caption if exists
    ...
    def __images__(self, fnm, zoomin=3, page_from=0,
        ...
        try:
            ...
            # 首先转换为图片，顺便提取字符信息（位置、内容、字体大小等）
            # 后续步骤主要基于图片和字符信息进行 布局分析 和 文本提取
        ...
        # 中英文采取不同的处理逻辑
        # 判断条件1：每页抽取的字符中如果有超过30个连续的英文字符（包括数字和常见标点），则认为是英文文档
        self.is_english = [re.search(r"[a-zA-Z0-9,/¸;:'\[\]\(\)!@#$%^&*\"?<>._-]{30,}", "".join(
            random.choices([c["text"] for c in self.page_chars[i]], k=min(100, len(self.page_chars[i]))))) for i in
                           range(len(self.page_chars))]
        # 判断条件2：如果超过一半的页数满足条件1，则认为整个文档是英文文档
        ...
        # st = timer()
        # 字符级分析
        # 针对英文文档丢失空格的问题，启发式地在“空白看起来比较大”的地方增加空格
        ...
        # boxes：识别的文本块，调用 OCR 后产生
        # 兜底判定：字符级判定失效时，利用 OCR 结果再次判定是否为英文文档
