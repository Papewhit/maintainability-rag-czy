...
The generated documentation catalog SHALL be human-readable navigation containing authority entry points, a regeneration command, a link to evidence-governance usage and status semantics, governed statuses, source fingerprint, clickable relative source links, and a `Global Finding Inbox` group derived from the same query semantics as `--finding-inbox`. It SHALL identify inbox entries as unconfirmed evidence.

#### Scenario: Catalog and console query are regenerated
- **WHEN** governed documentation validation regenerates the catalog
...
- **THEN** the header explains its navigation role, current-behavior authority, regeneration command, governance reference, and inbox evidence boundary
