...
- [x] 4.14 新增确定性 compiled-graph E2E，覆盖 intent classifier → ComprehensiveQueryPlan → baseline + sub-query 并行 dense/BM25 边界 → merge → 一次共享后处理 → 公共 trace；外部 LLM、embedding 与 Milvus 使用受控替身，不将其作为真实模型、release index 或成本/质量证据
