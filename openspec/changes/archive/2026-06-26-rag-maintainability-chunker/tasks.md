...
- [x] 9.1 实现 `backend/documents/normalizer/table_nearby.py`（复用 figure_normalizer 双策略：bbox proximity + text reference）
- [x] 9.2 修改 `normalizer/pipeline.py`：在 validate_and_enrich_tables 后调用 associate_nearby_blocks
- [x] 9.3 修改 `normalizer/__init__.py`：导出 associate_nearby_blocks
- [x] 9.4 修改 `converters.py`：table parent chunk 拼接 nearby texts（caption + nearby + markdown）
- [x] 9.5 修改 `_ocr.py`：recognize() 返回 `(text, score)` 元组
- [x] 9.6 修改 `_pdf_parser.py`：__ocr() 接收并存储 score 到 b["score"]
- [x] 9.7 修改 `adapter.py`：_convert_text_blocks 提取 score，_parse_pdf 计算 ocr_confidence_avg 和 parse_path
- [x] 9.8 修改 `base.py`：ParseMeta 添加 parse_path 字段
- [x] 9.9 修改 `models.py`：DocumentParseMeta 添加 parse_path 列
- [x] 9.10 修改 `document_service.py`：持久化 parse_path
- [x] 9.11 修改 `admin_documents.py`：Admin API 返回 parse_path
- [x] 9.12 OCR 试验：单元测试验证核心逻辑（表格编号提取、bbox proximity、parse_path 算法）

**验收：**
- ✅ Table nearby 关联逻辑正确（bbox proximity + text reference）
- ✅ parse_path 枚举和判断算法正确（native_text|ocr|mixed|unknown）
- ✅ 所有单元测试通过（6/6 tests passed）
- ✅ 现有测试无破坏（63 passed in normalizer + chunker）
- ✅ mypy 类型检查通过（table_nearby.py + adapter.py）
