...
- **THEN** query_entities 包含 `{type=component, canonical=主减速齿轮箱, surface=MRG}`、`{type=maintenance_action, canonical=拆卸, surface=拆卸}`；normalized_query 为 "主减速齿轮箱拆卸怎么做"；sparse_expansion 包含所有 variant 的并集
