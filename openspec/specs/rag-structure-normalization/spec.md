# rag-structure-normalization

文档结构规范化：在 ParsedDocument 之上构建标题层级树、识别并聚合 list_item、建立 figure 与附近段落的双向关联、校验并补漏表格 cells，最终输出 NormalizedDocument 作为 chunker 的输入。
